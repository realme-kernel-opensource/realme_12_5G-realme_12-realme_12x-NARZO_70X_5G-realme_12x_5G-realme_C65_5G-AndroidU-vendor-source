...
static int max77939_set_chg_auto_mode(struct oplus_voocphy_manager *chip, bool enable)
{
	...
	if (enable && (max77939_get_chg_auto_mode(chip) == MAX77939_CHG_FIX_MODE)) {
		/* not force automode to fix CP ERR_TRANS_DET_FLAG when enter SVOOC charging. */
		/* ret = max77939_update_bits(chip->client, MAX77939_REG_15, MAX77939_CHG_MODE_MASK, MAX77939_CHG_AUTO_MODE); */
	} else if (!enable && (max77939_get_chg_auto_mode(chip) == MAX77939_CHG_AUTO_MODE)) {
...
static int max77939_cp_set_work_start(struct oplus_chg_ic_dev *ic_dev, bool start)
{
	...
	if (start && (0 == (data & MAX77939_CHG_EN_MASK))) {
		/* disable the vac2v2x_ovpuvp to fix the ufcs not work issue. */
		rc = max77939_set_vac2v2x_ovpuvp_config(chip->voocphy, !start);
