...
/**
 *when the watchdog is set to 1s,the allowed error of the chip is 800ms. The time intervals between public
 *pps voltage regulation and detection phases are 200ms and 500ms, which are affected by system scheduling.
 *when watchdog timeout happen,cp will stop charge,all ibus will inflow on one cp,whill cause ibus ocp.
 *so need to open a work queue to kick the dog alone.any i2c communication can kick the dog.
 */
