...
static int oplus_pps_action_open_mos(struct oplus_pps_chip *chip)
{
	...
	schedule_delayed_work(&chip->kick_dog_work, msecs_to_jiffies(PPS_ACTION_KICK_DOG_DELAY));
...
static void oplus_pps_kick_dog_work(struct work_struct *work)
{
	struct oplus_pps_chip *chip = container_of(work, struct oplus_pps_chip, kick_dog_work.work);
...
static void oplus_pps_stop_work(struct work_struct *work)
{
	...
	if (chip->pps_adapter_type == PPS_ADAPTER_THIRD) {
		...
		oplus_pps_set_pdo(chip);
		cancel_delayed_work(&chip->kick_dog_work);
	}
...
int oplus_pps_init(struct oplus_chg_chip *g_chg_chip)
{
	...
	INIT_DELAYED_WORK(&chip->kick_dog_work, oplus_pps_kick_dog_work);
