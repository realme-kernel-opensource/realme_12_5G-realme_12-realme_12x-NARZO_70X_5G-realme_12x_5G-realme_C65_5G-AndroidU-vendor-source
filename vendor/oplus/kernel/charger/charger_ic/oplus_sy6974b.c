...
extern bool oplus_chg_is_support_qcpd(void);
...
int sy6974b_get_charger_vol(void)
{
	...
	chg_info("sy6974b_get_charger_vol: %d\n", vol_vaule);
	return vol_vaule;
	#else
...
int sy6974b_input_current_limit_write(int current_ma)
{
	...
	if (chip->charger_current_pre == current_ma) {
		pr_info("charger_current_pre == current_ma = %d.\n", current_ma);
		return 0;
	} else {
...
bool oplus_chg_is_usb_present(void)
{
	...
	if (oplus_voocphy_get_copycat_type() == FAST_COPYCAT_TYPE_UNKNOW)
		return pre_vbus_status;
	else
		return true;
}
...
static irqreturn_t sy6974b_irq_handler(int irq, void *data)
{
	...
	if (!prev_pg && chip->power_good) {
		...
		Charger_Detect_Release();
#endif
		if (g_oplus_chip->charger_volt >= g_oplus_chip->limits.charger_hv_thr) {
			pr_notice("adapter/usb ovp return, charger_volt[%d]\n", g_oplus_chip->charger_volt);
