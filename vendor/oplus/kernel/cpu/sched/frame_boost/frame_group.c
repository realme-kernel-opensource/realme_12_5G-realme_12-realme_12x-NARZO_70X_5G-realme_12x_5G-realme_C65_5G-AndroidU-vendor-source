...
unlock_fbg:
	...
	*util = max(raw_util, boosted_policy_util);
	...
	if (unlikely(sysctl_frame_boost_debug)) {
		...
		cpu_util_systrace_c(policy_util ? atomic64_read(&grp->curr_util) : 0, first_cpu,  "cfs_curr_util");
	...
	*util = max(*util, boosted_policy_util);
	...
	if (unlikely(sysctl_frame_boost_debug)) {
		...
		cpu_util_systrace_c(policy_util ? atomic64_read(&grp->curr_util) : 0, first_cpu, "rt_curr_util");
